...
void write(CServer &fd ,const vector<uint8_t> &arg)
{
    ...
    fd.UDPSendTo(&size, sizeof(size), ( sockaddr *)&sdd);
    fd.UDPSendTo((void*)arg.data(), arg.size(), ( sockaddr *)&sdd) ;
...
int main()
{
    ...
    sdd.sin_family = AF_INET; // IPv4
    sdd.sin_addr.s_addr = INADDR_ANY;
    sdd.sin_port = htons(2033);
    ...
    payload1.push_back(2);
    payload1.push_back(3);
    payload1.push_back(4);
