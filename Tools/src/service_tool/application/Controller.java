...
public class Controller
{
    ...
    public void initialize() {
    	...
    	iv.setFitHeight(400);
    	pane.setCenter(iv);
        indicator.appendText("[ARA::COM] Service Generator Tool");  
	...
	public void generate_mi_func(ActionEvent e) {
		...
		if(SIGroup.size()==0){
			indicator.clear();
	        indicator.setStyle("text-area-background: DarkSalmon;");
	        indicator.appendText("File is not a Service Interface!");  
	        refresh();
	        return;
		}
		for(Node SI : SIGroup) {
			if(generate_skeleton(SI) && generate_proxy(SI) && generate_common(SI) && generate_returntypes(SI)) {
				...
				indicator.clear();
		        indicator.setStyle("text-area-background: DarkSalmon;");
		        indicator.appendText("File is not a Service Interface, Missing Tags!");  
		        refresh();
		        return;
			}
		}
	...
	///////////////   Algorithm Code   ///////////////
	private boolean generate_skeleton(Node SI) {
		if(SI.Search("short-name").size()==0)return false;
	...
	private boolean generate_proxy(Node SI) {
		if(SI.Search("short-name").size()==0)return false;
		String name = SI.Search("short-name").get(0).getVal();
		if(name == null) return false;
		name = c_get(name);	
	...
	private boolean generate_common(Node SI) {
		if(SI.Search("short-name").size()==0)return false;
		String name = SI.Search("short-name").get(0).getVal();
		if(name == null) return false;
		name = c_get(name);	
	...
	private boolean generate_returntypes(Node SI) {
		if(SI.Search("short-name").size()==0)return false;
		String name = SI.Search("short-name").get(0).getVal();
		if(name == null) return false;
		name = c_get(name);		
	...
	private boolean WriteFile(String Path,String Text){
	...
	private String c_get(String in) {
		in = in.replaceAll(" ", "");	
		in = in.replaceAll("\t", "");	
		in = in.replaceAll("\n", "");
		return in;
